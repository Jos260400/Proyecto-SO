...
char buffer[LENGTH + 32] = {};
    ...
    while(1) {
        ...
        char* value_token = strtok(message, " ");
...
break;
    ...
    else if (strcmp(message, "Conectar") == 0)
    {
        sprintf(buffer, "Conectar %s: %s\n", nombre, message);
        printf("%s", buffer);
    }
...
printf("----> Cual su nombre?: ");
    fgets(nombre, 32, stdin);
    str_trim_lf(nombre, strlen(nombre));

if (strlen(nombre) > 32 || strlen(nombre) < 2){
printf("Probar de nuevo :)\n");
return EXIT_FAILURE;
}
